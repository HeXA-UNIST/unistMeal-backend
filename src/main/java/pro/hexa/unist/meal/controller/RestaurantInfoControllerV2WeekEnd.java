...
@RestController
@RequestMapping("/restaurantInfo/weekEnd")
@RequiredArgsConstructor
@Transactional
public class RestaurantInfoControllerV2WeekEnd {

    private static final Logger log = LoggerFactory.getLogger(RestaurantInfoControllerV2WeekEnd.class);
    ...
    private void uploadRestaurantInfo(String restaurantName, String breakfastStartTime, String breakfastEndTime, Integer breakfastPrice, Boolean breakfastIsOpened, String lunchStartTime, String lunchEndTime, Integer lunchPrice, Boolean lunchIsOpened, String dinnerStartTime, String dinnerEndTime, Integer dinnerPrice, Boolean dinnerIsOpened, String theKey) {
        ...
        if (breakfastIsOpened != null) {
            ...
            breakfast = new RestaurantInfoV2(restaurantName, breakfastPrice, true, Integer.parseInt(parsedBreakfastStartTime[0]), Integer.parseInt(parsedBreakfastStartTime[1]), Integer.parseInt(parsedBreakfastEndTime[0]), Integer.parseInt(parsedBreakfastEndTime[1]), "breakfast", DayType.WEEKEND);
        } else breakfast = new RestaurantInfoV2(restaurantName, false, "breakfast", DayType.WEEKEND);

        if (lunchIsOpened != null) {
            String[] parsedLunchStartTime = lunchStartTime.split(":");
            String[] parsedLunchEndTime = lunchEndTime.split(":");
            lunch = new RestaurantInfoV2(restaurantName, lunchPrice, lunchIsOpened, Integer.parseInt(parsedLunchStartTime[0]), Integer.parseInt(parsedLunchStartTime[1]), Integer.parseInt(parsedLunchEndTime[0]), Integer.parseInt(parsedLunchEndTime[1]), "lunch", DayType.WEEKEND);
        } else lunch = new RestaurantInfoV2(restaurantName, false, "lunch", DayType.WEEKEND);

        if (dinnerIsOpened != null) {
            String[] parsedDinnerStartTime = dinnerStartTime.split(":");
            String[] parsedDinnerEndTime = dinnerEndTime.split(":");
            dinner = new RestaurantInfoV2(restaurantName, dinnerPrice, dinnerIsOpened, Integer.parseInt(parsedDinnerStartTime[0]), Integer.parseInt(parsedDinnerStartTime[1]), Integer.parseInt(parsedDinnerEndTime[0]), Integer.parseInt(parsedDinnerEndTime[1]), "dinner", DayType.WEEKEND);
        } else dinner = new RestaurantInfoV2(restaurantName, false, "dinner", DayType.WEEKEND);

        repository.deleteByRestaurantNameAndDayType(restaurantName, DayType.WEEKEND);
